...
def generate_weather_data(device_id, timestamp, location):
    return{
        ...
        'airQualityIndex': random.uniform(0, 500)  #AGL value
    }
...
def create_kafka_producer(bootstrap_servers, max_retries=5, backoff_time=1):
    retries = 0
    while retries < max_retries:
        try:
            ...
            print(f"Kafka broker not available. Retrying in {backoff_time} seconds...")
            retries += 1
            sleep(backoff_time)
            backoff_time *= 2
    raise NoBrokersAvailable("Failed to connect to Kafka broker after multiple retries.")
...
if __name__ == "__main__":
    ...
    try:
        simulate_journey(producer, 'Vehicle-Kribit-789')
